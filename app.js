  ...
  const cards = boardData.plans.map((item, index) => {
    ...
    return `<article class="plan-item ${dirty ? 'is-dirty' : ''}" data-plan-card="${index}">
      ...
      <div class="plan-block">
        ...
        <div class="plan-operators">${operators.map((operator, operatorIndex) => `<div class="plan-operator ${operator.done ? 'is-done' : ''}"><input type="checkbox" data-plan-operator-done data-operator-index="${operatorIndex}" ${operator.done ? 'checked' : ''}><span class="operator-name editable-text" contenteditable="plaintext-only" data-plan-operator-name data-operator-index="${operatorIndex}" role="textbox">${escapeHtml(operator.name)}</span><button type="button" class="operator-remove" data-plan-remove-operator="${operatorIndex}" aria-label="删除算子 ${escapeHtml(operator.name)}">×</button></div>`).join('') || '<p class="operator-empty">尚未添加算子，点击“＋ 添加算子”补充。</p>'}</div>
      </div>
